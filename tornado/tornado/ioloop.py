...
class IOLoop(Configurable):
    ...
    def current(instance=True):
        ...
        如果调用过make_current就返回instance,如果没有当前的IOLoop,就
        返回IOLoop.instance()
    ...
    def close(self, all_fds=False):
        ...
        许多应用只有一个IOLoop,所以这个不是必须要调用的,因为当你Exit的时候,会自动释放这些资源
        提供此函数,或许对单元测试有一些用,因为可能需要进行大量的创建和释放
    ...
    def add_handler(self, fd, handler, events):
        ...
        在fd对应的socket上注册指定状态的handler,如果socket达到指定状态
        则此handler被调用
    ...
    def set_blocking_signal_threshold(self, seconds, action):
        ...
        注册信号处理器,当block超过了seconds时间的时候,就发送信号到action
        """
        raise NotImplementedError()
    ...
    def add_timeout(self, deadline, callback, *args, **kwargs):
        ...
        不应该在另外的线程里调用它,也就是他是线程不安全的,你必须用add_callback传递控制权
        到IOLoop的主线程,在那儿调用add_timeout
        ...
        .. versionchanged:: 4.0
           ...
           deadline到的时候自动调用callback,就是注册一个超时的回调函数
        """
    ...
    def call_at(self, when, callback, *args, **kwargs):
        ...
        在指定的时间调用callback,when必须是IOLoop.time
        """
    ...
    def remove_timeout(self, timeout):
        ...
        可以cancel从add_timeout等返回的handle
        """
        raise NotImplementedError()
    ...
    def add_callback(self, callback, *args, **kwargs):
        ...
        add_callback是唯一一个线程安全的,也就是说在任何线程任何时间点,都可以
        调用add_callback,而其他的所有交互,必须在IOLoop的线程里,add_callback
        能够被用来传递从另外线程到IOLoop的线程的控制
        """
        raise NotImplementedError()
